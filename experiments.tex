...
Kinect on our robot. If an object is in the view frustum and at least 50\% visible we assume a positive observation and observe its true pose with Gaussian noise.
% For objects that are in the view frustum, we get a false negative with probability proportional to the
...
distribution and then discard the sample with some probability. The probability with which we discard a sample is inversely proportional to the
probability that it lies in a negative region. The inverse
probability is given by the following heuristic: $\max(1,\dfrac{\text{penetration
...
Belief distributions containing many modes can be problematic under the \mld{} formulation. Consider a distribution with 100 modes scattered around a large room. It will take a really long time before the robot finds the object, especially if every observe action results in a new ML pose located far away from the current one. The same issue arises when searching for objects with large uniform distributions. All points on a uniform distribution are equally likely so the new ML location is chosen at random.

Another limitation is the use of rejection sampling. There is a noticeable slowdown in planning time when dealing with environments containing many geometric constraints.
